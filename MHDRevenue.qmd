...
receipts_income_plot <- rev_inc %>%
  ...
  annotate(geom="text",
           x=rev_inc %>% filter(Municipality=="Marblehead") %>% pull(DOR.Income.Per.Capita) + 30e3,
           y=rev_inc %>% filter(Municipality=="Marblehead") %>% pull(per_capita_receipts)-800,
           label="Marblehead") +
...
ggsave(here("figures", "receipts_income_plot.png"), receipts_income_plot, width=5, height=4, units="in")
