...
Marblehead ranks
`r max(dat$pci_rank) - mhd_percentiles$pci_rank + 1` 
...
`r max(dat$pc_receipts_rank) - mhd_percentiles$pc_receipts_rank + 1` of Massachusetts municipalities.
...
```{r}
dat %>%
  filter(pci_peer, pci_percentile > mhd_percentiles$pci_percentile)
...
Marblehead's per capita tax receipts of  `r label_dollar()(mhd_pc_receipts)`, are
...
dat %>%
  filter(Municipality=="Marblehead") %>%
  select(DOR.Income.Per.Capita, per_capita_receipts)
```
