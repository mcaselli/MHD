...
plot2 <- ggplot(inc_sal, aes(x = DOR.Income.Per.Capita, y = Average.Salary)) +
  geom_point(alpha = .5) +
  geom_smooth(method = "loess", se = FALSE, color = "#7CAE00") +
  theme_minimal()+
  geom_text_repel(data = inc_sal %>% filter( !(Municipality == "Marblehead")), aes(label = Municipality), max.overlaps = 20, size = 3, segment.color = 'grey80') +
  geom_label_repel(data = inc_sal %>% filter(Municipality == "Marblehead"), aes(label = Municipality), color = "#F8766D", position = position_nudge_repel(x =100, y =15))+
  geom_label_repel(data = inc_sal %>% filter(Municipality == "Gloucester" |Municipality == "Beverly"), aes(label = Municipality), color = "#3B9AB2") +
  geom_point(data = inc_sal %>% filter(Municipality == "Marblehead"), color = "#F8766D", size = 3) +
  geom_point(data = inc_sal %>% filter(Municipality == "Gloucester" |Municipality == "Beverly"), color = "#00BFC4", size = 2) +
...
```{r}
ggplot(rev_inc_sal, aes(x = DOR.Income.Per.Capita, y = Average.Salary)) +
  geom_point(alpha = .5) +
  geom_smooth(method = "loess", se = FALSE, color = "#7CAE00") +
  theme_minimal()+
